...
async function ytSearch(query, pageToken = '') {
  if (!YT_API_KEY) { showNoKey($('searchTrackList')); return; }
  $('searchTrackList').innerHTML = '<div class="loading-msg">Поиск...</div>';
  const data = await ytFetch('/search', {
    part: 'snippet', type: 'video', videoCategoryId: '10',
    maxResults: 20, q: query, ...(pageToken ? { pageToken } : {}),
  });
  if (data.error) { showToast('Ошибка API: ' + data.error.message); return; }
  nextPageToken = data.nextPageToken || '';
  $('btnMore').classList.toggle('hidden', !nextPageToken);
  const newT = data.items.map(ytItemToTrack);
...
async function ytLoadPopular() {
  if (!YT_API_KEY) { showNoKey($('homeTrackList')); return; }
  $('homeTrackList').innerHTML = '<div class="loading-msg">Загрузка...</div>';
  const data = await ytFetch('/videos', {
    part: 'snippet', chart: 'mostPopular',
    videoCategoryId: '10', maxResults: 20, regionCode: 'RU',
  });
  if (data.error) { showNoKey($('homeTrackList')); return; }
  tracks = data.items.map(item => ({
    id: item.id, name: item.snippet.title,
    artist: item.snippet.channelTitle,
    cover: item.snippet.thumbnails?.medium?.url, src: 'youtube',
  }));
...
async function ytLoadQuery(query, container) {
  ...
  try {
    const data = await ytFetch('/search', {
      part: 'snippet', type: 'video', videoCategoryId: '10', maxResults: 15, q: query,
    });
    if (data.error || !data.items?.length) {
      container.innerHTML = '<div class="loading-msg">Не удалось загрузить</div>';
      return [];
    }
    return data.items.map(ytItemToTrack);
  } catch (e) {
